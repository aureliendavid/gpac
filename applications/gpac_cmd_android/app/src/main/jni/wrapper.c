...
JNIEXPORT void JNICALL Java_com_gpac_cmd_CmdTerminal_run(JNIEnv * env, jobject obj, jstring sCommand, jstring sFile)
{
	//CAST_HANDLE(wr);
	jniLOGV("gpacterminal::start");
	/*if (!wr)
	{
		jniLOGV("gpacterminal::end : aborted");
	    return;
	}*/
	jboolean isCopy;
	...
	jniLOGV("gpacterminal::command get back ok");
	jniLOGV("%s\n", sOriginalCommand);
	...
	// FILE* ferr = freopen( sFile, "w", stderr );
	// FILE* fout = freopen( sFile, "w", stdout );

	FILE* ferr = freopen( "/mnt/sdcard/gpac.stderrout.txt", "w", stderr );
	FILE* fout = freopen( "/mnt/sdcard/gpac.stderrout.txt", "w", stdout );

	fprintf(fout, "test from wrapper\n");

	jniLOGV("gpacterminal calling main");
	gpac_main(i, sConvertedCommandLine);

	(*env)->ReleaseStringUTFChars(env, sCommand, sOriginalCommand);
	jniLOGV("gpacterminal::end");
...
char ** ConvertCommandLine( const char* sCommand, int* iNbArg )
{
	...
	strcpy( pReturn[0], "gpac" );//just a place holder , will never be read.
