...
static void ac4dmx_check_dur(GF_Filter *filter, GF_AC4DmxCtx *ctx)
{
	...
	// duration
	while (gf_ac4_parser_bs(bs, &hdr, GF_FALSE, GF_FALSE, GF_FALSE)) {
...
restart:
	...
	while (remain) {
		...
		res = gf_ac4_parser_bs(ctx->bs, &(ctx->hdr), is_first, GF_FALSE, GF_FALSE);
...
static const char *ac4dmx_probe_data(const u8 *_data, u32 _size, GF_FilterProbeScore *score)
{
	...
	while (gf_bs_available(bs) && nb_frames <= 4) {
		...
		if (!gf_ac4_parser_bs(bs, &ahdr, GF_TRUE, GF_FALSE, GF_FALSE)) {
